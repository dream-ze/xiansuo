...
from app.models import AccountCookieVersion, PlatformAccount, PublishAsset, PublishJob, Task, User
...
def _get_owned_publish_job(db: Session, current_user: User, job_id: int) -> PublishJob:
    job = db.scalars(select(PublishJob).where(PublishJob.id == job_id, PublishJob.user_id == current_user.id)).first()
    if job is None:
...
def _get_owned_publish_asset(db: Session, current_user: User, asset_id: int) -> PublishAsset:
    asset = db.scalars(
        ...
        .where(PublishAsset.id == asset_id, PublishJob.user_id == current_user.id)
    ).first()
    if asset is None:
...
def get_publish_jobs(
    ...
    statement = select(PublishJob).where(PublishJob.user_id == current_user.id)
    if platform:
...
def get_publish_assets(
    ...
    job = _get_owned_publish_job(db, current_user, job_id)
    assets = db.scalars(select(PublishAsset).where(PublishAsset.publish_job_id == job.id).order_by(PublishAsset.id.asc())).all()
    ...
    for asset in assets:
        if asset.upload_status in ("pending", "failed"):
            try:
                ...
                db.commit()
                raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail=f"素材上传失败: {exc}") from exc
    db.commit()
