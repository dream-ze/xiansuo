...
from app.models import AiDraft, DraftAsset, Note, NoteAsset, PlatformAccount, PublishAsset, PublishJob, User
...
def get_drafts(
    ...
    return success_response(paginated([_serialize_draft(draft) for draft in drafts], page, page_size))
...
def create_draft(
    ...
    db.commit()
    db.refresh(draft)
    return success_response(_serialize_draft(draft))
...
def send_draft_to_publish(
    ...
    db.commit()
    db.refresh(job)
    return success_response(_serialize_publish_job(job))
...
def delete_draft(
    ...
    db.delete(draft)
    db.commit()
    return success_response({"id": draft_id, "status": "deleted"})
...
def get_draft_assets(
    ...
    return success_response({"items": [_serialize_draft_asset(a) for a in assets]})
...
def add_draft_asset(
    ...
    db.commit()
    db.refresh(asset)
    return success_response(_serialize_draft_asset(asset))
...
def delete_draft_asset(
    ...
    db.delete(asset)
    db.commit()
    return success_response({"id": asset_id, "status": "deleted"})
