...
@router.get("/images")
def list_user_images(current_user: User = Depends(require_current_user)):
    ...
    return success_response({"items": files})


@router.delete("/images/{file_name}")
def delete_user_image(file_name: str, current_user: User = Depends(require_current_user)):
    prefix = f"xhs-upload-u{current_user.id}-"
...
def compose_image(payload: ComposeImageRequest, current_user: User = Depends(require_current_user)):
    from app.services.image_util import compose_cover_image
    ...
    return success_response(_serialize_media_file(file_name=file_name, width=payload.width, height=payload.height))


@router.post("/images/resize")
def resize_image(payload: ResizeImageRequest, current_user: User = Depends(require_current_user)):
    from app.services.image_util import resize_image_file
...
def download_export(file_name: str, current_user: User = Depends(require_current_user)):
    if Path(file_name).name != file_name or ".." in file_name:
...
@router.post("/upload")
async def upload_file(file: UploadFile, current_user: User = Depends(require_current_user)):
