...
export function AddAccountDrawer({ open, onClose, onBound }: AddAccountDrawerProps) {
  ...
  return (
    <Drawer
      title={
        <div>
          <div style={{ fontSize: 12, color: "rgba(0,0,0,0.45)", marginBottom: 4, textTransform: "uppercase", letterSpacing: 1 }}>
            XHS Account
          </div>
          <div style={{ fontSize: 18, fontWeight: 600, color: "rgba(0,0,0,0.88)" }}>添加小红书账号</div>
        </div>
      }
      placement="right"
      width={420}
      ...
      destroyOnClose
      styles={{
        header: { background: "#FFFFFF", borderBottom: "1px solid #E8E8E8" },
