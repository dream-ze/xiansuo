...
import { confirmXhsPhoneLogin, sendXhsPhoneCode } from "../../api/xhs-api";
...
export function PhoneLoginPanel({ accountType, onConfirmed }: PhoneLoginPanelProps) {
  ...
  return (
    <Space direction="vertical" size="middle" style={{ width: "100%" }}>
      <Text style={{ color: "rgba(0,0,0,0.65)" }}>{statusText}</Text>

      <Form layout="vertical">
        <Form.Item label={<span style={{ color: "rgba(0,0,0,0.88)" }}>手机号</span>}>
          <Input
            value={phone}
            onChange={(e) => setPhone(e.target.value)}
            placeholder="请输入手机号"
            style={{ background: "#FFFFFF", borderColor: "#E8E8E8", color: "rgba(0,0,0,0.88)" }}
          />
        </Form.Item>
      </Form>
      ...
      <Form layout="vertical">
        <Form.Item label={<span style={{ color: "rgba(0,0,0,0.88)" }}>验证码</span>}>
          <Input
            value={code}
