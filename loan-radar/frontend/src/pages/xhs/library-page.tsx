...
import { convertNotesToPosts } from "../../api/index";
import { formatShanghaiTime } from "../../lib/time";
import type { NoteAsset, NoteComment, NotesExportResponse, SavedNote, Tag as TagType } from "../../api/xhs-types";
...
export function XhsLibraryPage() {
  ...
  async function convertSingleToLeads() {
    if (!selectedNote) return; setIsCreatingDraft(true); setDetailActionMessage(null);
    try {
      const result = await convertNotesToPosts({ note_ids: [selectedNote.id] });
      setDetailActionMessage(
        `线索分析完成：成功 ${result.converted_count} 条，跳过 ${result.skipped_count} 条，失败 ${result.failed_count} 条`,
      );
    }
    catch { setDetailActionMessage("转线索分析失败。"); } finally { setIsCreatingDraft(false); }
  }
  ...
  async function batchConvertToLeads() {
    if (!selectedNoteIds.length) return;
    setIsBatchWorking(true); setBatchActionMessage(null);
    try {
      const result = await convertNotesToPosts({ note_ids: selectedNoteIds });
      setBatchActionMessage(
        `线索分析完成：成功 ${result.converted_count} 条，跳过 ${result.skipped_count} 条，失败 ${result.failed_count} 条`,
      );
    } catch { setBatchActionMessage("转线索分析失败。"); }
    finally { setIsBatchWorking(false); }
  }
  ...
  return (
    <div>
      ...
      <Row gutter={[16, 12]} style={{ marginBottom: 16 }}>
        <Col xs={12} sm={6}><Card size="small"><Statistic title="已保存笔记" value={total} /></Card></Col>
        <Col xs={12} sm={6}><Card size="small"><Statistic title="当前视图" value={viewMode === "card" ? "卡片" : "表格"} /></Card></Col>
        <Col xs={12} sm={6}><Card size="small"><Statistic title="已选择" value={selectedNoteIds.length} suffix="条" /></Card></Col>
        <Col xs={12} sm={6}><Card size="small"><Statistic title="平台" value="XHS" /></Card></Col>
      </Row>

      <Card size="small" style={{ marginBottom: 16 }}>
        <Row gutter={[12, 8]} align="middle">
          <Col xs={24} sm={12} md={5}><Input placeholder="标题、正文、作者" value={keywordFilter} onChange={(e) => setKeywordFilter(e.target.value)} allowClear /></Col>
          <Col xs={24} sm={12} md={4}><Select value={selectedTagFilter || undefined} onChange={(v) => setSelectedTagFilter(v ?? "")} placeholder="全部标签" allowClear style={{ width: "100%" }} options={availableTags.map((t) => ({ value: String(t.id), label: t.name }))} /></Col>
      ...
      {notes.length > 0 && (
        <Card size="small" style={{ marginBottom: 16 }}>
          <Space wrap>
            ...
            <Button icon={<RadarChartOutlined />} disabled={isBatchWorking || !selectedNoteIds.length} onClick={batchConvertToLeads} size="small">转线索分析</Button>
                ...
                <Card hoverable size="small" style={{ overflow: "hidden" }} onClick={() => void openDetail(note)}
                  cover={
                    <div style={{ position: "relative", background: "#F5F7FA" }}>
                      ...
                      {cover ? <img src={cover} alt={note.title} referrerPolicy="no-referrer" style={{ width: "100%", aspectRatio: "1/1", objectFit: "cover", display: "block" }} /> : <div style={{ width: "100%", aspectRatio: "1/1", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(0,0,0,0.15)", fontSize: 28 }}><PictureOutlined /></div>}
                  ...
                  <Card.Meta title={<Text ellipsis style={{ fontSize: 13 }}>{note.title || "未命名"}</Text>} description={
                    <>
                      ...
                      {(() => {
                        ...
                        return (
                          <div style={{ marginTop: 4, display: "flex", gap: 8, fontSize: 11, color: "rgba(0,0,0,0.45)" }}>
      ...
      <Drawer title={selectedNote?.title || "笔记详情"} open={isDetailOpen} onClose={closeDetail} width={640} styles={{ body: { background: "#FFFFFF" } }}>
        {selectedNote && (
          <Spin spinning={isDetailLoading}>
            ...
            <Space wrap style={{ marginBottom: 16 }}>
              ...
              <Button icon={<RadarChartOutlined />} onClick={convertSingleToLeads} loading={isCreatingDraft} size="small">转线索分析</Button>
                ...
                <Image.PreviewGroup>
                  <Space size={8} wrap>
                    {selectedAssets.map((a) => (
                      a.asset_type === "video" ? (
                        <div key={a.id} style={{ width: 80, height: 80, background: "#F5F7FA", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <Text strong>正文</Text>
              <Paragraph style={{ marginTop: 4, color: "rgba(0,0,0,0.65)", whiteSpace: "pre-wrap" }}>{selectedNote.content || "暂无正文。"}</Paragraph>
            </div>
            ...
            {isCommentsOpen && (
              <Card size="small" style={{ background: "#FFFFFF" }}>
                ...
                {topLevelComments.map((c) => (
                  <div key={c.comment_id} style={{ marginBottom: 10, paddingBottom: 8, borderBottom: "1px solid #E8E8E8" }}>
                    ...
                    {childComments(c.comment_id).map((r) => (
                      <div key={r.comment_id} style={{ marginLeft: 20, marginTop: 4, paddingLeft: 8, borderLeft: "2px solid #E8E8E8" }}>
            ...
            {selectedNote.raw_json && (
              <details style={{ marginTop: 16 }}>
                <summary style={{ cursor: "pointer", color: "rgba(0,0,0,0.45)", fontSize: 12 }}>原始 JSON</summary>
                <pre style={{ fontSize: 11, color: "rgba(0,0,0,.45)", background: "#FFFFFF", padding: 8, borderRadius: 6, overflow: "auto", maxHeight: 300 }}>{JSON.stringify(selectedNote.raw_json, null, 2)}</pre>
              </details>
            )}
          </Spin>
        )}
