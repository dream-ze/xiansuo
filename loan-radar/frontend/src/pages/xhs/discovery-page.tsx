...
import { fetchAccounts, fetchSavedNoteIds, fetchXhsNoteComments, fetchXhsNoteDetail, saveXhsNotesToLibrary, searchXhsNotes } from "../../api/xhs-api";
import type { NoteComment, PlatformAccount, XhsSearchNote, XhsSearchOptions } from "../../api/xhs-types";
  ...
  return (
    <div>
      <Row justify="space-between" align="middle" style={{ marginBottom: 24 }}>
        <Col xs={24} sm={undefined} flex="1"><Title level={4} style={{ margin: 0 }}>笔记发现</Title><Text type="secondary">按关键词或链接查找笔记，查看详情、评论和原文，保存到内容库</Text></Col>
        <Col xs={24} sm="auto" style={{ marginTop: 8 }}><Button icon={<ReloadOutlined />} onClick={loadAccounts} loading={isLoadingAccounts}>刷新账号</Button></Col>
      </Row>

      <Card style={{ marginBottom: 24 }}>
        <form onSubmit={(e) => { e.preventDefault(); void runSearch(1, false); }}>
          <Row gutter={[12, 12]}>
            <Col xs={24} sm={12} md={6}><div style={{ marginBottom: 4 }}><Text type="secondary" style={{ fontSize: 12 }}>搜索账号</Text></div><Select value={selectedAccountId} onChange={setSelectedAccountId} placeholder="选择 PC 账号" style={{ width: "100%" }} options={pcAccountOptions} /></Col>
            <Col xs={24} sm={12} md={6}><div style={{ marginBottom: 4 }}><Text type="secondary" style={{ fontSize: 12 }}>关键词</Text></div><Input.Search value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder="低卡早餐、通勤穿搭" loading={isSearching} onSearch={() => void runSearch(1, false)} enterButton /></Col>
            <Col xs={12} sm={8} md={3}><div style={{ marginBottom: 4 }}><Text type="secondary" style={{ fontSize: 12 }}>排序</Text></div><Select value={filters.sort_type_choice} onChange={(v) => setFilters((c) => ({ ...c, sort_type_choice: v }))} style={{ width: "100%" }} options={sortOptions} /></Col>
            <Col xs={12} sm={8} md={3}><div style={{ marginBottom: 4 }}><Text type="secondary" style={{ fontSize: 12 }}>类型</Text></div><Select value={filters.note_type} onChange={(v) => setFilters((c) => ({ ...c, note_type: v }))} style={{ width: "100%" }} options={noteTypeOptions} /></Col>
            <Col xs={12} sm={8} md={3}><div style={{ marginBottom: 4 }}><Text type="secondary" style={{ fontSize: 12 }}>时间</Text></div><Select value={filters.note_time} onChange={(v) => setFilters((c) => ({ ...c, note_time: v }))} style={{ width: "100%" }} options={noteTimeOptions} /></Col>
            <Col xs={12} sm={8} md={3}><div style={{ marginBottom: 4 }}><Text type="secondary" style={{ fontSize: 12 }}>范围</Text></div><Select value={filters.note_range} onChange={(v) => setFilters((c) => ({ ...c, note_range: v }))} style={{ width: "100%" }} options={noteRangeOptions} /></Col>
          </Row>
          <Row gutter={12} style={{ marginTop: 12 }} align="bottom">
            <Col xs={24} sm={12} md={6}><div style={{ marginBottom: 4 }}><Text type="secondary" style={{ fontSize: 12 }}>笔记 URL</Text></div><Input value={noteUrl} onChange={(e) => setNoteUrl(e.target.value)} placeholder="https://www.xiaohongshu.com/explore/..." /></Col>
                ...
                return (
                  <Col xs={12} sm={8} md={6} lg={4} xl={4} key={`${note.note_id}-${note.title}`}>
                    <Card hoverable size="small" style={{ overflow: "hidden" }} onClick={() => void openDetail(note)}
                      cover={
                        <div style={{ position: "relative", background: "#F5F7FA" }}>
                          {coverUrl
                            ...
                            : <div style={{ width: "100%", aspectRatio: "1/1", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(0,0,0,0.15)", fontSize: 28 }}><PictureOutlined /></div>}
                      ...
                      <Space size={12} style={{ marginTop: 8, fontSize: 12, color: "rgba(0,0,0,0.45)" }}>
                      ...
                      {commentPreviewByNoteId[note.note_id] && (
                        <div style={{ marginTop: 8, borderTop: "1px solid #E8E8E8", paddingTop: 8 }} onClick={stopCardClick}>
                          ...
                          {commentPreviewByNoteId[note.note_id].filter((c) => !c.parent_comment_id).slice(0, 4).map((c) => (
                            <div key={c.comment_id} style={{ marginBottom: 6, fontSize: 12 }}>
                              ...
                              <div style={{ color: "rgba(0,0,0,0.65)" }}>{c.content}</div>
                              {getChildComments(note.note_id, c.comment_id).map((r) => (
                                <div key={r.comment_id} style={{ marginLeft: 16, marginTop: 4 }}>
                                  ...
                                  <div style={{ color: "rgba(0,0,0,0.45)", fontSize: 12 }}>{r.content}</div>
                                </div>
                              ))}
                            </div>
                          ))}
      ...
      <Drawer title={selectedNote?.title || "笔记详情"} open={!!selectedNote} onClose={closeDetail} width={640} styles={{ body: { background: "#FFFFFF" } }}>
        {selectedNote && (
          <div>
            ...
            {selVideoUrl && getNoteKindLabel(selectedNote) === "视频" ? (
              <div style={{ marginBottom: 16 }}>
                <div style={{ position: "relative", background: "#F5F7FA", borderRadius: 8, overflow: "hidden" }}>
                  {selImgUrls.length ? <img src={selImgUrls[0]} alt="视频封面" referrerPolicy="no-referrer" style={{ width: "100%", maxHeight: 400, objectFit: "contain", display: "block" }} /> : <div style={{ height: 200, display: "flex", alignItems: "center", justifyContent: "center" }}><PlayCircleOutlined style={{ fontSize: 40, color: "rgba(0,0,0,0.25)" }} /></div>}
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ position: "relative", background: "#F5F7FA", borderRadius: 8, overflow: "hidden", textAlign: "center" }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <Text strong>正文</Text>
              <Paragraph style={{ marginTop: 4, color: "rgba(0,0,0,0.65)" }}>{selectedNote.content || "暂无正文。"}</Paragraph>
            </div>
            ...
            {commentPreviewByNoteId[selectedNote.note_id] && (
              <Card size="small" title="评论预览" style={{ background: "#FFFFFF" }}>
                ...
                {commentPreviewByNoteId[selectedNote.note_id].filter((c) => !c.parent_comment_id).map((c) => (
                  <div key={c.comment_id} style={{ marginBottom: 12, paddingBottom: 8, borderBottom: "1px solid #E8E8E8" }}>
                    ...
                    <div style={{ color: "rgba(0,0,0,0.65)", fontSize: 13, marginTop: 2 }}>{c.content}</div>
                    {getChildComments(selectedNote.note_id, c.comment_id).map((r) => (
                      <div key={r.comment_id} style={{ marginLeft: 20, marginTop: 6, paddingLeft: 8, borderLeft: "2px solid #E8E8E8" }}>
