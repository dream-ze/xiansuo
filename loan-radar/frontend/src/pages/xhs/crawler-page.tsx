...
import { crawlXhsDataStream, fetchAccounts } from "../../api/xhs-api";
import type { PlatformAccount, XhsDataCrawlItem, XhsDataCrawlMode } from "../../api/xhs-types";
  ...
  return (
    <div>
      <Row justify="space-between" align="middle" style={{ marginBottom: 24 }}>
        ...
        <Col>
          <Space>
            <Link to="/collection"><Button size="small">线索雷达采集</Button></Link>
      ...
      <Card style={{ marginBottom: 24 }}>
        <Form layout="vertical" onFinish={() => void handleRun()}>
          <Row gutter={[16, 12]}>
            ...
            <Col xs={12} sm={6} md={4} style={{ display: "flex", alignItems: "center", paddingTop: 8 }}>
          ...
          {mode === "search" ? (
            <Row gutter={[16, 12]}>
              <Col xs={24} md={8}><Form.Item label="搜索关键词"><Input value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder="低卡早餐、通勤穿搭" /></Form.Item></Col>
              <Col xs={12} sm={6} md={4}><Form.Item label="爬取数量"><InputNumber min={1} max={200} value={maxNotes} onChange={(v) => { const n = v ?? 20; setMaxNotes(n); setPages(Math.max(1, Math.ceil(n / 20))); }} style={{ width: "100%" }} /></Form.Item></Col>
              <Col xs={12} sm={6} md={4}><Form.Item label="排序"><Select value={filters.sort_type_choice} onChange={(v) => setFilters((c) => ({ ...c, sort_type_choice: v }))} options={sortOptions} /></Form.Item></Col>
              <Col xs={12} sm={6} md={4}><Form.Item label="类型"><Select value={filters.note_type} onChange={(v) => setFilters((c) => ({ ...c, note_type: v }))} options={noteTypeOptions} /></Form.Item></Col>
              <Col xs={12} sm={6} md={4}><Form.Item label="时间范围"><Select value={filters.note_time} onChange={(v) => setFilters((c) => ({ ...c, note_time: v }))} options={noteTimeOptions} /></Form.Item></Col>
              <Col xs={12} sm={6} md={4}><Form.Item label="距离"><Select value={filters.pos_distance} onChange={(v) => setFilters((c) => ({ ...c, pos_distance: v }))} options={distanceOptions} /></Form.Item></Col>
              <Col xs={12} sm={6} md={4}><Form.Item label="Geo"><Input value={filters.geo} onChange={(e) => setFilters((c) => ({ ...c, geo: e.target.value }))} placeholder="经纬度" /></Form.Item></Col>
            </Row>
          ) : (
