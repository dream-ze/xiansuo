...
5. 评分规则可编辑和测试。
6. 仪表板全局统计可查看。
7. 智能工作流：知识库索引、合规规则、工作流运行、审批队列、ReAct 智能体对话可演示。

## 前端导航

左侧导航栏分 4 个区域：

- **线索雷达**（上部）：工作台、采集中心、帖子池、线索池、CRM 跟进、同行发现、获客报告
- **小红书运营**（中部）：运营总览、账号矩阵、笔记发现、数据抓取、关键词组、数据洞察、图片工坊、视频工坊、内容库、草稿工坊、发布中心、自动运营
- **智能工作流**（中下部）：AI 助手、知识库、合规规则、工作流、审批中心
- **通用**（底部）：任务中心、模型配置、系统设置

- 采集中心页面包含"监控源"和"采集任务"两个 Tab
...
3. 需要已登录用户账号（JWT Token）
4. 需要配置有效的 AI 模型（文本模型，用于 RAG 和智能体）
5. 需要已有笔记/帖子数据（用于知识库索引）
...
1. **知识库索引**
   ...
   - 点击"索引笔记"，选择笔记批量索引到 ChromaDB
   - 等待索引完成，查看新增条目数

2. **知识库检索与问答**
   - 在"语义检索"输入框输入查询（如"征信花了怎么贷款"）
   - 查看返回的相关知识条目和相似度分数
   - 在"知识问答"输入框输入问题（如"征信花了还能申请贷款吗？"）
   - 查看基于知识库的 AI 回答和参考来源

3. **合规规则管理**
   - 进入"合规规则"页面（`/compliance-rules`）
   - 点击"初始化默认规则"，一键创建 9 条默认规则
   - 查看规则列表：3 条平台规则 + 3 条行业规范 + 3 条内部策略
   - 创建自定义合规规则（如"不得使用'包下款'等绝对化用语"）

4. **AI 助手对话**
   - 进入"AI 助手"页面（`/agent`）
   - 输入问题："帮我生成一条针对'急需5万周转'客户的跟进话术"
   - 观察智能体工具调用过程：
     - 调用 knowledge_search 检索知识库参考素材
     - 调用 script_generate 生成话术候选
     - 调用 compliance_check 检查合规性
   - 查看最终回答（包含话术和合规检查结果）
   - 继续对话："再换一个更委婉的风格"（多轮对话续接）
...
5. **工作流运行**
   - 通过 API 触发线索评分工作流：
     ...
     curl -X POST http://localhost:8001/api/workflows/run \
       ...
       -d '{"workflow_type": "lead_scoring", "input_data": {"lead_id": 1}}'
     ```
   - 通过 API 触发话术生成工作流：
     ```bash
     curl -X POST http://localhost:8001/api/workflows/run \
       -H "Authorization: Bearer <token>" \
       -H "Content-Type: application/json" \
       -d '{"workflow_type": "script_generation", "input_data": {"lead_id": 1}}'
     ```
   - 进入"工作流"页面（`/workflows`），查看运行记录和节点日志

6. **审批队列**
   - 如果话术生成工作流检测到高风险，自动创建审批记录
   - 进入"审批中心"页面（`/approvals`）
   - 查看待审批列表（按风险等级排序）
   - 查看合规检查详情和风险原因
   - 通过审批（可修改内容后通过）或拒绝
   - 审批通过后，工作流自动恢复执行
